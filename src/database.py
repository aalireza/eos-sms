...
class BackEnd:
    ...
    def get_balance(self, **args):
        ...
        name = self.user_info.find_one({"num" : args.get('From')}).get('wallet').get('name')
        return 'Balance: %s' %Commands._GetBalance(admin_name = self.admin.name, of = name), None
    ...
    def send(self, **args):
        ...
        return True
        '''
        from_ = self.user_info.find_one({"num" : args.get('From')}).get('wallet').get('name')
        to = self.user_info.find_one({"num" : args.get('To')}).get('wallet').get('name')
        admin_name, amount = self.admin.name, "{:.04f}".format(float(args.get('amt')))
        print(admin_name)
        print(from_)
        ...
        if success:
            return "\nsent %s from your wallet to %s \nRemaining Balance: %s" %(args.get('amt'), args.get('To'), self.get_balance(From = args.get('From'))), None
        else:
    ...
    def create(self, **args):
        ...
        self.user_info.update_one({"num" : From},
                              {"$set": {"num" : From,
                               "seed" : seed,
                               "qr" : qr_location,
                               "wallet" : Commands.Create(admin = self.admin)
                               }}, upsert=True)
        '''
